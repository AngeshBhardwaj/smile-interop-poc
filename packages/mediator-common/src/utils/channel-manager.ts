 ...
 * Programmatically creates channels from mediator configuration
 */

import axios from 'axios';
import https from 'https';
import { getLogger } from './logger';
import type { OpenHIMConfig, MediatorConfig } from './registration';

const logger = getLogger('channel-manager');

/**
 * Create default channels for a mediator
 * @param openhimConfig - OpenHIM core configuration
 * @param mediatorConfig - Mediator configuration containing defaultChannelConfig
 */
export async function createDefaultChannels(
  ...
  try {
    logger.info({
      msg: 'Starting channel creation process',
