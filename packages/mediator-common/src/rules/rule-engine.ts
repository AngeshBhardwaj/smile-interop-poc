...
import type { CloudEvent } from '../config/types';

/**
 * Get cache settings from environment
 */
function getCacheSettings(): { enableCaching: boolean; cacheTTL: number } {
  return {
    enableCaching: process.env.RULE_CACHE_ENABLED !== 'false', // Default: true
    cacheTTL: parseInt(process.env.RULE_CACHE_TTL || '300', 10), // Default: 300 seconds
  };
}
...
export function getRules(forceReload = false): TransformationRule[] {
  const now = Date.now();
  const { enableCaching, cacheTTL } = getCacheSettings();
  const cacheTTLMs = cacheTTL * 1000; // Convert to milliseconds
  ...
  if (
    ...
    enableCaching &&
    ruleCache.size > 0 &&
    now - lastCacheRefresh < cacheTTLMs
  ) {
    // Return cached rules
