/**
 * Custom Transformation Mediator - Main Entry Point
 *
 * OpenHIM mediator that transforms CloudEvents to Custom JSON format
 * for Warehouse client using JSONPath-based field mapping
 */
...
async function start(): Promise<void> {
  try {
    logger.info({
      msg: 'Starting Custom Transformation Mediator',
      version: '1.0.0',
    ...
    const server = app.listen(config.service.port, () => {
      logger.info({
        msg: 'Custom Transformation Mediator HTTP server started',
        port: config.service.port,
        endpoints: [
    ...
    logger.info({
      msg: 'Custom Transformation Mediator is ready',
      port: config.service.port,
      rulesLoaded: rulesResult.rules.length,
      targetClient: config.client.name,
      targetEndpoint: config.client.endpoint,
    });
    ...
    logger.fatal({
      msg: 'Failed to start Custom Transformation Mediator',
      error: error.message,
      stack: error.stack,
    });
    process.exit(1);
